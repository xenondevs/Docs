...
| `outer_wall_distance` (optional in Json, defaults to a random int. Either `4` or `5`)  | An [`IntProvider`](../placed-feature.md#int-providers). (Range limit in Json is $[1;20]$) | The offset of each coordinate from the feature origin.                                                                    | Random `int` between `4` and `6` |

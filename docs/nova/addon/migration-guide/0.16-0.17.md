...
Instead, everything registered in code now.
...
There were also some changes to the `Provider<T>` system. You now need to call `get()` instead of `.value` to access the value.
...
The few utility functions Nova still provides for interacting with nms enchantment classes have been consciously removed

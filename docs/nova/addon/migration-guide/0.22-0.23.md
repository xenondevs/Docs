...
* In `BlockBehavior`, `handleInteract` was replaced with `use` and `useItemOn`. Check out the updated [item behavior](../items/item-behaviors.md) and [block behavior](../blocks/block-behaviors.md) pages for a detailed overview of the new interaction system.
